...
class PathSpaceFactory(PathSpaceFactoryType):
    def __init__(self, map_space: MapSpace) -> None:
        ...
        self._path_nodes: Dict[MapNode, PathNode] = {}

    def generate_edges(self) -> List[PathEdge]:
        path_edges = []

        for map_edge in self._map_space.edges:
            _1 = self._path_nodes[map_edge.pair[0]]
            _2 = self._path_nodes[map_edge.pair[1]]
    ...
    def generate_nodes(self) -> List[PathNode]:
        for map_node in self._map_space.nodes:
            self._path_nodes[map_node] = PathNode(map_node)

        return list(self._path_nodes.values())

    def generate_defaults(self) -> Tuple[PathNode, PathNode]:
        from_node = self._path_nodes[self._map_space.from_node]
        to_node = self._path_nodes[self._map_space.to_node]
